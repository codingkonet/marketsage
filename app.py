...
MODEL_TYPES = ["random_forest", "xgboost", "neural_net", "ensemble"]
...
def forex():
    ...
    form_data = {"pair": "EURUSD=X", "period": "1y", "interval": "1d",
                 "model_type": "random_forest", "walk_forward": ""}
    ...
    if request.method == "POST":
        form_data.update({
            ...
            "walk_forward": request.form.get("walk_forward", ""),
            "compare":      request.form.get("compare", ""),
        })
        try:
            agent = ForexAgent(
                pair=form_data["pair"], period=form_data["period"],
                interval=form_data["interval"], model_type=form_data["model_type"],
            )
            result = agent.analyze(
                run_walk_forward=bool(form_data.get("walk_forward")),
                run_comparison=bool(form_data.get("compare")),
            )
        except Exception as exc:
            error = str(exc)

    return render_template("forex.html", form_data=form_data, result=result,
                           error=error, model_types=MODEL_TYPES)
    ...
    if request.method == "POST":
        ...
        for pair in pairs:
            try:
                ...
                r = agent.analyze(run_backtest=False, run_sentiment=False)
        ...
        if len(pairs) > 1:
            ...
            if len(closes.columns) > 1:
                ...
                corr_data = {"labels": list(corr.columns), "values": corr.values.tolist()}

    return render_template("dashboard.html", form_data=form_data, results=results,
                           corr_data=corr_data, error=error)
...
def api_price(symbol):
    try:
        ...
        return jsonify({"symbol": symbol, "price": price, "change_pct": chg})
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
def api_compare():
    ...
    try:
        ...
        from forex_agent.agent import _compare_models
        data = build_features(load_forex_data(pair, period, interval))
        return jsonify({"comparison": _compare_models(data)})
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
def export_options():
    ...
    try:
        ...
        buf = io.StringIO()
        if r.calls:
            fields = ["type"] + list(r.calls[0].keys())
